<script setup lang="ts">
import Axios from "axios";
import { computed, reactive, ref } from "vue";
import { useRouter } from "vue-router";

const router = useRouter();

// -------- config API --------
// ถ้าใน .env มี VITE_API_BASE → ใช้ค่านั้น
// ถ้าไม่มี → ใช้ default "https://exam-api.dev.mis.cmu.ac.th/api"
const API_BASE =
  (import.meta.env.VITE_API_BASE as string) ||
  "https://exam-api.dev.mis.cmu.ac.th/api";

// endpoint ของ blogs
const BLOGS_INDEX =
  (import.meta.env.VITE_API_BLOGS_INDEX as string) || "/blogs";

// header แนบ token (ถ้า login แล้วมี token เก็บใน localStorage)
const AUTH_HEADER: Record<string, string> = {
  Authorization: `Bearer ${localStorage.getItem("token") || ""}`,
};

// ประเภทข้อมูลที่รับจาก API เวลา error
import type { ApiErrorPayload } from '@/models/common';

// -------- state ฟอร์ม --------
type Form = { title: string; content: string; file: File | null };
const form = reactive<Form>({ title: "", content: "", file: null });

// touched = ใช้บอกว่าผู้ใช้เคย "แตะ" field นั้นหรือยัง → จะได้ไม่โชว์ error ตั้งแต่แรก
const touched = reactive({ title: false, content: false, file: false });

const loading = ref(false);         // โหลดอยู่หรือไม่
const apiError = ref<string | null>(null); // ข้อความ error จาก API
const preview = ref<string | null>(null);  // รูป preview เวลาอัปโหลด

/* -------- validation -------- */
const errors = computed(() => {
  const e = { title: "", content: "", file: "" };

  // ตรวจ title
  if (!form.title.trim()) e.title = "กรุณากรอกหัวข้อ";
  else if (form.title.trim().length < 3)
    e.title = "หัวข้อต้องยาวอย่างน้อย 3 ตัวอักษร";

  // ตรวจ content
  if (!form.content.trim()) e.content = "กรุณากรอกเนื้อหา";
  else if (form.content.trim().length < 10)
    e.content = "เนื้อหาต้องยาวอย่างน้อย 10 ตัวอักษร";

  // ตรวจไฟล์รูป
  if (form.file) {
    if (!form.file.type.startsWith("image/"))
      e.file = "อัปโหลดได้เฉพาะไฟล์รูปภาพ";
    else if (form.file.size > 2 * 1024 * 1024)
      e.file = "ขนาดไฟล์สูงสุด 2 MB";
  }
  return e;
});

// form valid = ต้องไม่มี error
const isValid = computed(
  () => !errors.value.title && !errors.value.content && !errors.value.file
);

// handler: เวลาเลือกไฟล์
function onPick(e: Event) {
  const f = (e.target as HTMLInputElement).files?.[0] || null;
  form.file = f;
  touched.file = true;
  preview.value = f ? URL.createObjectURL(f) : null; // สร้าง URL preview ชั่วคราว
}

// handler: เวลา submit form
async function onSubmit() {
  // mark ว่ากรอกแล้ว
  touched.title = touched.content = true;
  apiError.value = null;

  if (!isValid.value) return; // ถ้ามี error → ไม่ส่ง

  try {
    loading.value = true;
    const fd = new FormData();
    fd.append("title", form.title.trim());
    fd.append("content", form.content.trim());
    if (form.file) fd.append("blog_img", form.file); // ตาม API ต้องใช้ชื่อ blog_img

    await Axios.post(`${API_BASE}${BLOGS_INDEX}`, fd, {
      headers: { ...AUTH_HEADER, "Content-Type": "multipart/form-data" },
    });

    // สำเร็จ → กลับไปหน้า blogs
    router.push({ name: "blogs" });
  } catch (e: unknown) {
    let msg = "บันทึกไม่สำเร็จ";
    if (Axios.isAxiosError<ApiErrorPayload>(e))
      msg =
        e.response?.data?.message ??
        e.response?.data?.error ??
        e.message ??
        msg;
    else if (e instanceof Error) msg = e.message;
    else msg = String(e);
    apiError.value = msg;
  } finally {
    loading.value = false;
  }
}
</script>

<template>
  <div class="bg-white shadow rounded-xl p-6">
    <form class="space-y-5" @submit.prevent="onSubmit">
      <label class="block mb-3 font-bold">เพิ่มบทความ</label>

      <!-- title -->
      <div>
        <label class="block mb-1 font-medium">หัวข้อ <span class="text-red-500">*</span></label>
        <input v-model="form.title" @blur="touched.title = true" type="text"
          class="w-full rounded-lg border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="ใส่หัวข้อบทความ" />
        <p v-if="touched.title && errors.title" class="text-sm text-red-600 mt-1">
          {{ errors.title }}
        </p>
      </div>

      <!-- content -->
      <div>
        <label class="block mb-1 font-medium">เนื้อหา<span class="text-red-500">*</span></label>
        <textarea v-model="form.content" @blur="touched.content = true" rows="8"
          class="w-full rounded-lg border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="พิมพ์เนื้อหา…" />
        <p v-if="touched.content && errors.content" class="text-sm text-red-600 mt-1">
          {{ errors.content }}
        </p>
      </div>

      <!-- image -->
      <div>
        <label class="block mb-1 font-medium">รูปภาพ</label>
        <input type="file" accept="image/*" @change="onPick" class="block w-full" />
        <p v-if="touched.file && errors.file" class="text-sm text-red-600 mt-1">
          {{ errors.file }}
        </p>
        <img v-if="preview" :src="preview" alt="preview" class="mt-3 h-32 rounded-lg object-cover" />
      </div>

      <!-- error จาก API -->
      <div v-if="apiError" class="text-red-600 text-sm">{{ apiError }}</div>

      <!-- ปุ่มบันทึก -->
      <div class="pt-2">
        <button type="submit" :disabled="loading || !isValid"
          class="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-medium px-6 py-2.5 rounded-lg">
          {{ loading ? "กำลังบันทึก..." : "บันทึก" }}
        </button>
      </div>
    </form>
  </div>
</template>
