...
<template>
  ...
  <div class="flex flex-col items-end gap-2">
    ...
    <div class="flex items-center gap-2">
      ...
      <!-- @click="$emit('pin')" เมื่อคลิกปุ่ม จะ ส่งอีเวนต์ชื่อ pin ออกไปให้พาเรนต์  ตัวปุ่มเองไม่เปลี่ยนสถานะ; แค่แจ้งพาเรนต์ว่า “ผู้ใช้กดปักหมุดแล้วนะ” พาเรนต์ต้องเป็นคนจัดการต่อ เช่น toggle ค่า, call API แล้วอัปเดต prop pinned-->
      <button
        :class="
          props.pinned
      ...
      <label class="flex items-center cursor-pointer gap-2">
        ...
        <span
          ...
          :class="props.active ? 'text-green-600' : 'text-gray-500'"
        >
    ...
    <div class="flex items-center gap-2">
      ...
      <button
        ...
        "
        :title="props.active ? 'ซ่อนเพื่อลบ' : 'ลบ'"
        @click="!props.active && $emit('delete')"
      >
        🗑
      </button>
    </div>
